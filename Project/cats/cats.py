...
def choose(paragraphs, select, k):
    ...
    for i in paragraphs:
        if select(i):
            True_paragraghs.append(i)
    if len(True_paragraghs) < k + 1:
        return ''
    else:
        return True_paragraghs[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    def about_helper(s):
        for i in topic:
            if i in split(remove_punctuation(lower(s))):
                return True
        return False
    return about_helper
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        corrected_word = user_word
        min_diff = limit
        for i in valid_words:
            if diff_function(user_word, i, limit) < min(min_diff, limit) or diff_function(user_word, i, limit) == limit and corrected_word == user_word:
                min_diff = diff_function(user_word, i, limit)
                corrected_word = i
...
def shifty_shifts(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helper(w1, w2, diff):
        if diff + abs(len(start) - len(goal)) > limit:
            return limit + 1
        if w1 == "" or w2 == "":
            return diff
        elif w1[0] != w2[0]:
            return helper(w1[1:], w2[1:], diff + 1)
        else:
            return helper(w1[1:], w2[1:], diff)
    return helper(start, goal, 0) + abs(len(start) - len(goal))
    # END PROBLEM 6
